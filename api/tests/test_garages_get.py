...
class GaragesGetTestCase(TestCase):
    def setUp(self):
    ...
    def test_garage_get_valid(self):
        ...
        response = self.client.get('/api/garages/', HTTP_AUTHORIZATION='Token ' + self.user_token.key)
    ...
    def test_garage_get_valid_no_garages(self):
        response = self.client.get('/api/garages/', HTTP_AUTHORIZATION='Token ' + self.user_token.key)
        response_content = json.loads(response.content)
    ...
    def test_garage_get_invalid_not_authenticated(self):
        ...
        response = self.client.get('/api/garages/')
        response_content = json.loads(response.content)
